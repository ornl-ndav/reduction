 ...
 * $Id$
 *
 * \file utils/test/cpp/shift_spectrum_test.cpp
 */
...
/**
 * \defgroup shift_spectrum_test shift_spectrum_test
 * \{
 *
 * This test compares the output data (\f$output\f$) calculated by the library
 * function <i>shift_spectrum</i> and described in section 3.64 of the
 ...
 * shift
 * \param axis_bc (OUTPUT) is the array of bin centers from the independent 
 * values
                       ...
                       Nessi::Vector<NumT> & input_err2)
{
  ...
  axis.push_back(static_cast<NumT>(4.0));
  axis.push_back(static_cast<NumT>(5.0));
  axis.push_back(static_cast<NumT>(6.0));

  axis_bc.push_back(static_cast<NumT>(1.5));
  axis_bc.push_back(static_cast<NumT>(2.5));
  axis_bc.push_back(static_cast<NumT>(3.5));
  axis_bc.push_back(static_cast<NumT>(4.5));
  axis_bc.push_back(static_cast<NumT>(5.5));
...
/**
 * Function that generates the data using the <i>shift_spectrum</i> function 
 * (as described in the documentation of the <i>shift_spectrum</i> function) 
 * and launches the comparison of the data. Returns the result of the 
 * test_okay function (TRUE/FALSE).
 *
...
{ 
  ...
  Nessi::Vector<NumT> input;
  Nessi::Vector<NumT> input_err2;
  NumT xshift = static_cast<NumT>(3.5);
  NumT xmin = static_cast<NumT>(1.0);
  NumT xmax = static_cast<NumT>(6.0);
  ...
  initialize_inputs(axis, axis_bc, input, input_err2);
  initialize_true_outputs(true_output, true_output_err2);
 
  // run the code being tested
  Utils::shift_spectrum(input, input_err2, axis, axis_bc, xshift, xmin, xmax,
                        output, output_err2);
  ...
  if (!debug.empty())
    {
      ...
      print(output_err2, true_output_err2, VV, debug);
    }
...
/**
 * Main function that tests <i>shift_spectrum</i> for float and double
 *
...
/**
 * \}
 */   //end of shift_spectrum_test group
