...
int main() 
{
  ...
  float f_a_err2 = 3.0;
  float f_a_2;
  vector<float> f_x;                //Original vector
  vector<float> f_x_err2;            //Original vector err
  ...
  ArrayManip::div_scalar_vec_ncerr(f_x, f_x_err2, f_y, f_y_err2, f_a, f_a_err2);
  ArrayManip::div_scalar_vec_ncerr(d_x, d_x_err2, d_y, d_y_err2, d_a, d_a_err2);
  ArrayManip::div_scalar_vec_ncerr(i_x, i_x_err2, i_y, i_y_err2, i_a, i_a_err2);
  ArrayManip::div_scalar_vec_ncerr(u_x, u_x_err2, u_y, u_y_err2, u_a, u_a_err2);
  ...
  for (int i=0; i<n; i++)
    {
      ...
      f_a_2 = f_x[i]*f_x[i];
      f_true_vector_err2[i] = (((f_a*f_a)/(f_a_2*f_a_2))*f_x_err2[i])+(f_a_err2/f_a_2);

      d_a_2 = d_x[i]*d_x[i];
      d_true_vector_err2[i] = (((d_a*d_a)/(d_a_2*d_a_2))*d_x_err2[i])+(d_a_err2/d_a_2);

      i_a_2 = i_x[i]*i_x[i];
      i_true_vector_err2[i] = (((i_a*i_a)/(i_a_2*i_a_2))*i_x_err2[i])+(i_a_err2/i_a_2);

      u_a_2 = u_x[i]*u_x[i];
      u_true_vector_err2[i] = (((u_a*u_a)/(u_a_2*u_a_2))*u_x_err2[i])+(u_a_err2/u_a_2);

    }

  //check first the size
  if ((f_x.size() != f_y.size())||(d_x.size() != d_y.size()) || (i_x.size() != i_y.size()) || (u_x.size() != u_y.size()))
    {
  ...
  cout << "div_scalar_vec_ncerr_test.cpp..........";
